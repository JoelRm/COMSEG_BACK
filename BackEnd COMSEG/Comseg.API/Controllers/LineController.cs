...
namespace Comseg.API.Controllers
{
    ...
    public class LineController : ControllerBase
    {
        ...
        [HttpGet]
        [ProducesResponseType(typeof(BaseResponseEntity<ICollection<LineInfo?>>), 200)]
        ...
        [HttpGet("{id:int}")]
        [ProducesResponseType(typeof(BaseResponseEntity<LineInfo?>), 200)]
        [ProducesResponseType(typeof(BaseResponseEntity<LineInfo?>), 404)]
        ...
        public async Task<IActionResult> Post(DtoLine request)
        {
            var userId = HttpContext.User.Claims.FirstOrDefault(p => p.Type == ClaimTypes.Sid);
            if (userId == null) return Unauthorized();

            var response = await _service.CreateAsync(request, userId.Value);
            return Created($"api/Lines/{response.ResponseResult}", response);
        }

        [HttpPut("{id:int}")]
        [ProducesResponseType(typeof(BaseResponse), 200)]
        [ProducesResponseType(typeof(BaseResponse), 400)]
        public async Task<IActionResult> Put(int id, DtoLine request)
        {
            var userId = HttpContext.User.Claims.FirstOrDefault(p => p.Type == ClaimTypes.Sid);
            if (userId == null) return Unauthorized();
