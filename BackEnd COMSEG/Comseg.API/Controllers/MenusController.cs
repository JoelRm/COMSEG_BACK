...
namespace Comseg.API.Controllers
{
    ...
    public class MenusController : ControllerBase
    {
        ...
        [HttpGet]
        [ProducesResponseType(typeof(BaseResponseEntity<ICollection<MenuInfo?>>), 200)]
        ...
        [HttpGet("{id:int}")]
        [ProducesResponseType(typeof(BaseResponseEntity<MenuInfo?>), 200)]
        [ProducesResponseType(typeof(BaseResponseEntity<MenuInfo?>), 404)]
        ...
        public async Task<IActionResult> Post(DtoMenu request)
        {
            var userId = HttpContext.User.Claims.FirstOrDefault(p => p.Type == ClaimTypes.Sid);
            if (userId == null) return Unauthorized();
        ...
        public async Task<IActionResult> Put(int id, DtoMenu request)
        {
            var userId = HttpContext.User.Claims.FirstOrDefault(p => p.Type == ClaimTypes.Sid);
            if (userId == null) return Unauthorized();
