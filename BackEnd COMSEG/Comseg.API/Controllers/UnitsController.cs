...
namespace Comseg.API.Controllers
{
    ...
    public class UnitsController : ControllerBase
    {
        ...
        [HttpGet]
        [ProducesResponseType(typeof(BaseResponseEntity<ICollection<UnitInfo?>>), 200)]
        ...
        [HttpGet("{id:int}")]
        [ProducesResponseType(typeof(BaseResponseEntity<UnitInfo?>), 200)]
        [ProducesResponseType(typeof(BaseResponseEntity<UnitInfo?>), 404)]
        ...
        public async Task<IActionResult> Post(DtoUnit request)
        {
            var userId = HttpContext.User.Claims.FirstOrDefault(p => p.Type == ClaimTypes.Sid);
            if (userId == null) return Unauthorized();
        ...
        public async Task<IActionResult> Put(int id, DtoUnit request)
        {
            var userId = HttpContext.User.Claims.FirstOrDefault(p => p.Type == ClaimTypes.Sid);
            if (userId == null) return Unauthorized();
